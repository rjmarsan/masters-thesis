...
AndroMEDA, however, can easily detect this example. \temp{Demonstrate this. with screenshots}. Seen in Figure \ref{somethingthatdoesntexistyet}, we demonstrate the malware without AndroMEDA, and in Figure \ref{fig:infotheft_visual}, with AndroMEDA. After the user logs in, AndroMEDA immediately alerts the user that the PII has been read. The user then reviews it and decides that it's malware.
